function [server_sch,app_sch,complete,reward_ratio] = schedule_PDAGTO_origin(priority,App,topo,reward,first,parti)
    ...
    while any([priority{:}])
        ...
        %while any(rank{k})
            ...
            if f_sch <= dead(1,k)
                ...
                %0909 check
                for xxx=1:size(server_sch(m_sch).core(r_sch).schedule,1)
                    if s_sch > server_sch(m_sch).core(r_sch).schedule(xxx,3) && s_sch < server_sch(m_sch).core(r_sch).schedule(xxx,4)
                        CHECK = 1;
                    elseif f_sch > server_sch(m_sch).core(r_sch).schedule(xxx,3) && f_sch < server_sch(m_sch).core(r_sch).schedule(xxx,4)
                        CHECK = 1;
                    end    
                end 
                ...
                if ID_sch ~= 0
                    ...
                    if server_sch(m_sch).core(r_sch).idle(ID_sch,1) == server_sch(m_sch).core(r_sch).idle(ID_sch,2)
