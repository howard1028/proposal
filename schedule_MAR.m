function [server_sch,app_sch,complete,reward_ratio] = schedule_MAR(priority,rank,App,topo,reward,first,parti)
%UNTITLED13 Summary of this function goes here
%   Detailed explanation goes here

%此函數用的全域
global App topo n_a n_b a cores dead ft reward
...
while any(priority)
    ...
    while any(rank{k})
        ...
        for m = first(1,k):n_b
            ...
            prede_stmp = -1 ;
            for i = 1:pre
                if app_sch(k).task(prede(i)).part(1,4) > prede_stmp
                    prede_stmp = app_sch(k).task(prede(i)).part(1,4)  ;
                end    
            end
